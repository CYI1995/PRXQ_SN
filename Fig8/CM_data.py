...
eps = 0
basis = np.load('SP_basis_d16.npy')
L = int(math.sqrt(len(basis)))
...
for n in range(Samples):
    ...
    for i in range(L):
        for j in range(L):
            idx = i*L + j
            T_cor[i][j] = np.trace(rho_temp.dot(basis[idx])).real/d

    for i in range(7):
        k = klist[i]
        if(moment_based_CM(T_cor,k) == 0):
            break 
        else:
...
np.save('Ratiolist_CM_d16e0.npy',Ratiolist_CM)
plt.plot(klist,Ratiolist_CM,label = 'eps = 0')
